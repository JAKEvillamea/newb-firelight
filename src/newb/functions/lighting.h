...
vec3 nlLighting(
  ...
  if (env.nether || env.end) {
    ...
    // torch light
    light += torchLight*(2.0-(max(shadow, 1.65*lit.y)*dayFactor*(1.0-0.3*env.rainFactor)));
  }

 // Optimized darken at crevices
float col_max = max(COLOR.r, max(COLOR.g, COLOR.b));

// Use smoothstep instead of branching
float darkFactor = mix(0.24, 0.3, smoothstep(0.35, 0.7, COLOR.g));
light *= mix(1.0, darkFactor, step(col_max, 0.7));
